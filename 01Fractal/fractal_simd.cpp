...
#include <immintrin.h> // trabajar con avx

extern int max_iteraciones;
extern std::complex<double> c;

 // LA RELACION PUEDE HABER MAS DE UNA VEZ PERO LA IMPLEMENTACION
void julia_serial_simd(double x_min, double y_min, double x_max, double y_max, uint32_t width, uint32_t height, uint32_t* pixel_buffer){
    double dx = (x_max - x_min) / width;
    double dy = (y_max - y_min) / height;
    //AVX2 -> 8 floats (32 bits)
    // -1.5 x 8
    __m256 xmin = _mm256_set1_ps(x_min); // vector
    ...
    for(int i = 0 ; i < width ; i++){
        for(int j = 0 ; j < height ; j+=8){
            ...
            // verificcacion de los complejos si esta acotados (cr, ci)
            int iter = 1;
            ...
            while(iter < max_iteraciones){
                ...
                _mm256_add_ps(_mm256_sub_ps(zr2,zi2), c_real); 
                _mm256_add_ps(_mm256_sub_ps(zrzi,zrzi), c_imag); 

                //normas
                _mm256_mul_ps(zr, zr);
                _mm256_mul_ps(zi, zi);
                __m256 norma2 =_mm256_add_ps(zr2, zi2);
                // si la norma 2 < 4.0f devuelve 0xFFFFFFFF salieron, sino 0X00000000
                __m256 mask = _mm256_cmp_ps(norma2, max_norma,_CMP_LE_OS);

                mk = _mm256_and_ps(_mm256_and_ps(mask, one), mk);
